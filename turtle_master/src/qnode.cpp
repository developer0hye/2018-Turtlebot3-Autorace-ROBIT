...
#define THESH_RIGHT_DISTANCE 0.22

#define SAFETY_MODE 0
#define SAFETY_TUNNEL_MODE 0
...
void QNode::visionMsgCallback(const turtle_vision::vision_msg::ConstPtr& vision_msg)
{
    ...
    case LifeDriving::TURTLE_TRAFFIC_LIGHT_CHECK:
        ...
        if(vision_msg->signal == GREEN_LIGHT)
        {
...
void QNode::laserMsgCallback(const sensor_msgs::LaserScan &laser_arr)
{
    ...
    rightDistances.push_back(laser_arr.ranges.at(270));

    for(int i = 0 ; i < 3 ; i++)
    {
        rightDistances.push_back(laser_arr.ranges.at(271 + i));
        rightDistances.push_back(laser_arr.ranges.at(269 - i));
    }

    double meanRightDistance = 0.0;
    int    nValidRightDistance = 0;

    for(int i = 0; i < rightDistances.size(); i++)
    {
        if(rightDistances[i]> 0.001)
        {
            meanRightDistance += rightDistances[i];
            nValidRightDistance++;
        }
    }

    if(nValidRightDistance != 0)
    {
        meanRightDistance /= (double)nValidRightDistance;
        life_driving.lidarRightDistance = meanRightDistance;
        std::cout << "life_driving.lidarRightDistance = " << life_driving.lidarRightDistance << std::endl;
    }
...
//    goal_pose.pose.orientation.w = 0.7092063831855171;
//    goal_pose.pose.orientation.z = -0.7050009262752196;
    goal_pose.pose.orientation.w = 0.994225208543;
    goal_pose.pose.orientation.z = -0.107313720916;
    goal_pose.pose.position.x = goal_pos_x;
    goal_pose.pose.position.y = goal_pos_y;
    goal_pose_pub.publish(goal_pose);
    goal_pose_pub.publish(goal_pose);
...
//        std::cout << "noEyeState = " << noEyeState << std::endl;
    ...
    if(life_driving.arrive_goal  && tunnelEscapeState == TUNNEL_GO )
    {
        if(life_driving.lidarRightDistance < THESH_RIGHT_DISTANCE)
        {
            life_driving.setVelocity(0.22, 0.0);
            cmd_vel_pub.publish(life_driving.cmd_vel_msg);
            tunnelEscapeState = TUNNEL_ESCAPE;
...
void QNode::rvizInit()
{
    system("roslaunch turtlebot3_navigation turtlebot3_navigation.launch map_file:=$HOME/map6.yaml &");
//    initial_pose.header.frame_id = "map";
//    initial_pose.header.stamp = ros::Time::now();
//    initial_pose.pose.pose.orientation.w = imu_w;
//    initial_pose.pose.pose.orientation.z = imu_z;
